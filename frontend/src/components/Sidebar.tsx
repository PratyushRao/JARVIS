...
export default function Sidebar({ isOpen, activeChatId, onSelectChat, onNewChat, onClose }: SidebarProps) {
  ...
  const saveRename = async (chatId: string) => {
    ...
    try {
      ...
      console.error("Sidebar reload after rename failed", e);
    }
  };
  ...
  const handleDelete = async (chatId: string) => {
    if (confirm("Are you sure you want to delete this memory?")) {
      ...
      try {
        ...
        console.error("Sidebar reload after delete failed", e);
      }
    }
  };
  ...
  return (
    <div style={{
        ...
        {chats.map((chat) => (
          <div 
            ...
            style={{
              ...
              borderRadius: '6px',
              cursor: 'pointer',
              background: activeChatId === chat.chat_id ? 'rgba(255, 0, 0, 0.2)' : 'transparent',
              border: activeChatId === chat.chat_id ? '1px solid red' : '1px solid transparent',
          ...
          >
            ...
            {editingChatId === chat.chat_id ? (
              <div style={{ display: 'flex', flex: 1, gap: '5px' }} onClick={e => e.stopPropagation()}>
                <input 
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && saveRename(chat.chat_id)}
            ...
            <div 
              onClick={(e) => { e.stopPropagation(); setMenuOpenId(menuOpenId === chat.chat_id ? null : chat.chat_id); }}
              style={{ padding: '4px' }}
            >
            ...
            {menuOpenId === chat.chat_id && (
              <div ref={menuRef} style={{
                ...
                <div 
                  onClick={(e) => { e.stopPropagation(); handleDelete(chat.chat_id); }}
      ...
      <div style={{ padding: '20px', borderTop: '1px solid #333' }}>
         <button onClick={onClose} className="btn btn--ghost full-width">
           CLOSE PANEL
         </button>
      </div>
    </div>
